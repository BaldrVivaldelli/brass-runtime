...
| `src/core/runtime` | `npm run test:types`; `npm test -- src/core/runtime/__tests__` | engine parity tests; scheduler/fiber/scope invariants |
...
| benchmarks | `npm run benchmark`; `npm run benchmark:json`; related tests | benchmark thresholds and warnings |

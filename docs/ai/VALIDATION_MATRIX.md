...
| `crates/brass-engine-core`, `crates/brass-runtime-wasm-engine` | `npm run rust:check`; `npm run rust:fuzz:check`; `npm run build:wasm`; `npm test -- src/core/runtime/__tests__/engine src/core/runtime/__tests__/scheduler` | ABI/IPC handshake and limits, Rust code, generated bridge shape, shared fixtures |

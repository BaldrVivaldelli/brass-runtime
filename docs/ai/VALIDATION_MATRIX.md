...
- `npm run validate:products` packs and installs the runtime plus Agent/Perf
  companion packages in an isolated consumer, then checks ESM, CJS, types, and
  both CLIs.
...
Product-level lanes can be run independently before the full release gate:
...
| `src/perf`, `packages/perf` | `npm run test:types:perf`; `npm run test:perf`; after build, `npm run validate:product:perf`; `npm run perf -- --profile runtime`; `npm run perf:history -- --profile runtime`; `npm run perf:runtime:ab`; `npm run perf:runtime:soak`; `npm run perf:runtime:budget`; `npm run perf:http:memory -- --calls 1000 --concurrency 64 --warmup 0 --variants default-json,default-json-observed`; optional `node --expose-gc --import tsx src/perf/cli.ts --profile http-memory --calls 1000 --concurrency 64 --force-gc` | `docs/performance-profiler.md`; `docs/ai/PUBLIC_API.md` |
...
| `src/agent`, `packages/agent` | `npm run test:types:agent`; `npm run test:agent`; after build, `npm run validate:product:agent`; `npm run agent:test:smoke` | `docs/agent-boundaries.md`; `docs/product-boundaries.md`; config/policy docs |
...
| `extensions/vscode-brass-agent` | `npm ci --prefix extensions/vscode-brass-agent`; `npm run compile --prefix extensions/vscode-brass-agent`; `npm run package:vsix --prefix extensions/vscode-brass-agent` | `.github/workflows/vscode.yml`; VS Code install/clean docs |

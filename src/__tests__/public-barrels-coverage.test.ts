...
describe("public barrel modules", () => {
  it("load the runtime, core, engine, HTTP, compression, and lifecycle barrels", async () => {
    ...
    expect(root.Layer.services({ svc: root.defineService("Svc") })).toMatchObject({ _tag: "Sync" });
    expect(root.Layer.test(root.defineService("Svc"), { ok: true })).toMatchObject({ _tag: "Layer" });
    expect(root.Layer.tests([root.defineService("Svc"), { ok: true }])).toMatchObject({ _tag: "Layer" });
    expect(root.Layer.all(root.Layer.succeed({ one: 1 }), root.Layer.succeed({ two: 2 }))._tag).toBe("Layer");
    expect(root.mergeAll(root.Layer.succeed({ one: 1 }), root.Layer.succeed({ two: 2 }))._tag).toBe("Layer");
    expect(root.Layer.composeAll(root.Layer.succeed({ one: 1 }), root.Layer.succeed({ two: 2 }))._tag).toBe("Layer");
    expect(root.composeAll(root.Layer.succeed({ one: 1 }), root.Layer.succeed({ two: 2 }))._tag).toBe("Layer");
    expect(root.useService).toBe(core.useService);
    expect(root.useServices).toBe(core.useServices);
    expect(root.getServices).toBe(core.getServices);
    expect(root.makeConfigLayer).toBe(core.makeConfigLayer);
    expect(root.makeRuntimeLayer).toBe(core.makeRuntimeLayer);
    expect(root.RuntimeService).toBe(core.RuntimeService);
    expect(root.makeTestLayer).toBe(core.makeTestLayer);
    expect(root.makeTestLayers).toBe(core.makeTestLayers);
    ...
    expect(http.HttpClientService).toMatchObject({ _tag: "ServiceTag", name: "HttpClient" });
    expect(http.makeDefaultHttpClientLayer).toBeTypeOf("function");
    ...
    expect(httpTesting.makeMockDefaultHttpClient).toBeTypeOf("function");
    expect(httpTesting.makeMockDefaultHttpClientLayer).toBeTypeOf("function");
    ...
    expect(observability.ObservabilityService).toMatchObject({ _tag: "ServiceTag", name: "Observability" });
    expect(observability.makeObservabilityLayer).toBeTypeOf("function");
    expect(observability.makeObservedHttpClientLayer).toBeTypeOf("function");
